...
const DEFAULT_CONFIG_PATH = (_a = process.env.LOADTEST_AGENT_CONFIG) !== null && _a !== void 0 ? _a : '${escapedConfigPath}';
const DEFAULT_METADATA_PATH = (_b = process.env.LOADTEST_AGENT_METADATA) !== null && _b !== void 0 ? _b : '${escapedMetadataPath}';
const DEFAULT_API_URL = '${escapedApiUrl}';
const AGENT_VERSION = '${agentVersion}';
const AGENT_FILE_PATH = (_c = process.env.LOADTEST_AGENT_BINARY_PATH) !== null && _c !== void 0 ? _c : '${escapedBinaryPath}';
...
function encryptEnvelopePayload(key, payload) {
    ...
    const plaintext = Buffer.from(JSON.stringify(payload !== null && payload !== void 0 ? payload : {}), 'utf8');
...
function verifySignedDocument(scope, document) {
    ...
    const serialized = JSON.stringify(payload !== null && payload !== void 0 ? payload : {});
...
function deriveKeyFromMetadata(metadata) {
    var _a;
    const keyB64 = (_a = metadata === null || metadata === void 0 ? void 0 : metadata.derived_key) !== null && _a !== void 0 ? _a : FALLBACK_DERIVED_KEY_B64;
...
function normalizeConfigShape(source) {
    ...
    return {
        ...
        telemetryIntervalMinutes: Number(config.telemetryIntervalMinutes || config.telemetry_interval_minutes || 30),
        updateIntervalMinutes: Number(config.updateIntervalMinutes || config.update_interval_minutes || DEFAULT_UPDATE_INTERVAL_MINUTES),
        refreshIntervalMinutes: Number(config.refreshIntervalMinutes ||
            config.refresh_interval_minutes ||
            CONFIG_REFRESH_INTERVAL_MINUTES),
        featureFlags: (config.featureFlags && typeof config.featureFlags === 'object' ? config.featureFlags : {}) ||
            {},
...
function deriveIntervals(config) {
    var _a, _b, _c, _d;
    return {
        pollIntervalMs: Math.max(5, Number((_a = config.pollIntervalSeconds) !== null && _a !== void 0 ? _a : 30)) * 1000,
        telemetryIntervalMs: Math.max(1, Number((_b = config.telemetryIntervalMinutes) !== null && _b !== void 0 ? _b : 30)) * 60 * 1000,
        updateIntervalMs: Math.max(10, Number((_c = config.updateIntervalMinutes) !== null && _c !== void 0 ? _c : DEFAULT_UPDATE_INTERVAL_MINUTES)) *
            60 *
            1000,
        configRefreshIntervalMs: Math.max(5, Number((_d = config.refreshIntervalMinutes) !== null && _d !== void 0 ? _d : CONFIG_REFRESH_INTERVAL_MINUTES)) *
            60 *
            1000
    };
}
...
function handleConfigCommand(argv) {
    ...
    const nextConfig = {
        ...
        telemetryIntervalMinutes: Number(args['telemetry-interval'] || current.telemetryIntervalMinutes || 30),
        updateIntervalMinutes: Number(args['update-interval'] || current.updateIntervalMinutes || DEFAULT_UPDATE_INTERVAL_MINUTES),
        refreshIntervalMinutes: Number(args['refresh-interval'] || current.refreshIntervalMinutes || CONFIG_REFRESH_INTERVAL_MINUTES),
...
function mergeRemoteConfig(current, remote) {
    ...
    const next = {
        ...
        pollIntervalSeconds: Number((_a = settings.pollIntervalSeconds) !== null && _a !== void 0 ? _a : current.pollIntervalSeconds) || current.pollIntervalSeconds,
        telemetryIntervalMinutes: Number((_b = settings.telemetryIntervalMinutes) !== null && _b !== void 0 ? _b : current.telemetryIntervalMinutes) ||
            current.telemetryIntervalMinutes,
        updateIntervalMinutes: Number((_c = settings.updateIntervalMinutes) !== null && _c !== void 0 ? _c : current.updateIntervalMinutes) ||
            current.updateIntervalMinutes,
        refreshIntervalMinutes: Number((_d = settings.refreshIntervalMinutes) !== null && _d !== void 0 ? _d : current.refreshIntervalMinutes) ||
            current.refreshIntervalMinutes,
        featureFlags: (settings.featureFlags && typeof settings.featureFlags === 'object'
...
async function main() {
    ...
    while (true) {
        try {
            ...
            logError('Error:", error.message);, sessionToken = null);
            await sleep(Math.min(intervals.pollIntervalMs, 10000));
        }
    }
}
main().catch((error) => {
    logError('Fatal error:", error);, process.exit(1));
});
`;
}
;
